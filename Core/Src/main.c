...
#define T4_BALL_SERVO_MAX_US                      ((int32_t)2000)
...
static uint32_t T4Ball_PixelsToMillimetersCeil(uint32_t pixels)
{
    /* Calibration from Task 3: 43 px is approximately 10 mm. */
    return (pixels * 10U + 42U) / 43U;
}
...
bool Task4MainBall_Update(const VisionStatus_t *vision_status)
{
    ...
    error_px = center_x - T4_BALL_TARGET_CX;
    frame_delta_px = 0;
    raw_velocity_px_s = 0;
    ...
    s_t4_ball_status.within_one_cm =
        (abs_error_px <= T4_BALL_ONE_CM_PX);

    if ((uint32_t)abs_error_px > s_t4_ball_status.max_abs_error_px)
    {
        s_t4_ball_status.max_abs_error_px = (uint32_t)abs_error_px;
        s_t4_ball_status.max_abs_error_mm =
            T4Ball_PixelsToMillimetersCeil((uint32_t)abs_error_px);
    }
    ...
    if ((error_px != 0) && (filtered_velocity_px_s != 0))
    {
        ...
        {
            position_control_us =
                T4Ball_MulDivI32(position_control_us, 3, 4);
        }
        else
        {
            position_control_us =
                T4Ball_MulDivI32(position_control_us, 5, 4);
        }
    }
...
{
    if (!s_t4_ball_initialized)
    {
        return;
    }

    (void)BSP_Servo_SetPulseUs((uint16_t)T4_BALL_SERVO_CENTER_US);
    BSP_Servo_Disable();
    s_t4_ball_initialized = false;
