...
void AppTaskPort_ForceSafeStop(void)
{
    ...
    /*
     * Reset/fault/menu-reset must leave the rod at the calibrated horizontal
     * position.  Do this last because individual task stop paths may disable
     * or rewrite the servo while they release their own resources.
     */
    if ((!BSP_Servo_IsInitialized()) && (!BSP_Servo_Init()))
    {
        return;
    }
    (void)BSP_Servo_SetPulseUs(PROJECT_SERVO_HORIZONTAL_US);
    (void)BSP_Servo_Enable();
}
