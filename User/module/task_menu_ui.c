...
static TaskMenuTask_t s_selected_task = TASK_MENU_TASK_2_LAP_STOP;
static TaskMenuState_t s_state = TASK_MENU_STATE_BOOT;
...
static uint32_t s_last_render_decisecond = UINT32_MAX;
...
static void TaskMenu_CopyText(char *destination, const char *source)
{
    uint8_t index = 0U;
    ...
    if (source != NULL)
    {
        while ((source[index] != '\0') &&
               (index < (TASK_MENU_STATUS_TEXT_SIZE - 1U)))
        {
...
static TaskMenuTask_t TaskMenu_NextTask(TaskMenuTask_t task)
{
...
static void TaskMenu_DrawTaskLine(uint8_t page, TaskMenuTask_t task)
{
    BSP_Oled_DrawString(0U, page, (task == s_selected_task) ? ">" : " ");
    BSP_Oled_DrawString(12U, page, TaskMenuUi_TaskName(task));
}

static void TaskMenu_DrawElapsed(uint8_t page)
{
    uint32_t seconds = s_elapsed_ms / 1000U;
    uint32_t decisecond = (s_elapsed_ms / 100U) % 10U;
...
static void TaskMenu_RenderSelfCheck(void)
{
    ...
    BSP_Oled_DrawString(0U, 3U,
        BSP_Oled_IsOnline() ? "OLED: OK" : "OLED: RETRY");
    BSP_Oled_DrawString(0U, 5U, s_status_text);

    if ((s_warning_mask & APP_WARNING_TASK_PLACEHOLDER) != 0U)
    {
        BSP_Oled_DrawString(0U, 7U, "TASK: SKELETON");
    }
}
...
static void TaskMenu_RenderSelect(void)
{
    ...
    TaskMenu_DrawTaskLine(1U, TASK_MENU_TASK_2_LAP_STOP);
    TaskMenu_DrawTaskLine(2U, TASK_MENU_TASK_3_BALL_SEQUENCE);
    TaskMenu_DrawTaskLine(3U, TASK_MENU_TASK_4_AB_HOLD);
    TaskMenu_DrawTaskLine(4U, TASK_MENU_TASK_5_LAP_HOLD);
    TaskMenu_DrawTaskLine(5U, TASK_MENU_TASK_6_LAP_TARGET);
    BSP_Oled_DrawString(0U, 7U, "UP:NEXT K0:OK");
}
...
static void TaskMenu_RenderRunning(void)
{
    ...
    BSP_Oled_DrawString(0U, 1U, TaskMenuUi_TaskName(s_selected_task));
    ...
    if ((s_warning_mask & APP_WARNING_TASK_PLACEHOLDER) != 0U)
    {
        BSP_Oled_DrawString(0U, 6U, "NO MOTION OUTPUT");
    }
...
static void TaskMenu_RenderCurrent(void)
{
    ...
    {
        OledFullscreenTimer_ShowElapsedMs(s_elapsed_ms);
    }
    else
    {
        switch (s_state)
    ...
    s_last_render_decisecond = s_elapsed_ms / 100U;
}

bool TaskMenuUi_Init(void)
{
    s_selected_task = TASK_MENU_TASK_2_LAP_STOP;
    ...
    s_last_render_decisecond = UINT32_MAX;

    TaskMenu_CopyText(s_status_text, "SAFE SKELETON");
    TaskMenu_CopyText(s_fault_name, "NONE");
...
void TaskMenuUi_Process(void)
{
    ...
    switch (s_state)
    {
        case TASK_MENU_STATE_SELECT:
            ...
            {
                (void)BSP_Key_TakePress(BSP_KEY_CONFIRM);
                s_selected_task = TaskMenu_NextTask(s_selected_task);
                s_selection_change_count++;
                s_display_dirty = true;
            }
            else if (BSP_Key_TakePress(BSP_KEY_CONFIRM))
            {
                TaskMenu_ClearRequests();
    ...
    now_ms = HAL_GetTick();
    current_decisecond = s_elapsed_ms / 100U;

    if (((s_state == TASK_MENU_STATE_STARTING) ||
         (s_state == TASK_MENU_STATE_RUNNING) ||
         (s_state == TASK_MENU_STATE_STOPPING)) &&
        (current_decisecond != s_last_render_decisecond) &&
        ((uint32_t)(now_ms - s_last_render_ms) >= APP_UI_RUNTIME_REFRESH_MS))
    {
        s_display_dirty = true;
    }
...
{
    if ((!s_initialized) || (s_state == state))
    {
        return;
    }
...
{
    uint32_t old_tenth = s_elapsed_ms / 100U;
    uint32_t new_tenth = elapsed_ms / 100U;

    s_elapsed_ms = elapsed_ms;
    if (old_tenth != new_tenth)
    {
        s_display_dirty = true;
    }
}
...
void TaskMenuUi_SetStatusText(const char *text)
{
    ...
    memcpy(previous, s_status_text, sizeof(previous));
    TaskMenu_CopyText(s_status_text, text);

    if (memcmp(previous, s_status_text, sizeof(previous)) != 0)
    {
        s_display_dirty = true;
    }
}
...
{
    if ((!s_initialized) || (s_fullscreen_timer_enabled == enabled))
    {
        return;
    }
    ...
    const char *fault_name)
{
    ...
    TaskMenu_CopyText(s_fault_name, fault_name);
...
{
    if ((!s_initialized) || (!s_start_request))
    {
        return false;
    }
...
{
    if ((!s_initialized) || (!s_stop_request))
    {
        return false;
    }
    s_stop_request = false;
...
{
    if ((!s_initialized) || (!s_menu_request))
    {
        return false;
    }
    s_menu_request = false;
...
{
    if ((!s_initialized) || (!s_reset_request))
    {
        return false;
    }
    s_reset_request = false;
...
{
    TaskMenuUi_SetState(TASK_MENU_STATE_FINISHED);
}
...
bool TaskMenuUi_GetStatus(TaskMenuStatus_t *status)
{
    ...
    status->selection_change_count = s_selection_change_count;
    status->start_request_count = s_start_request_count;
    status->stop_request_count = s_stop_request_count;
    status->menu_request_count = s_menu_request_count;
    status->reset_request_count = s_reset_request_count;
    return true;
}
...
void TaskMenuUi_SetRunningElapsedMs(
    ...
    bool timer_running)
{
    TaskMenuUi_SetElapsedMs(elapsed_ms);

    if (!timer_running)
    {
        if (s_selected_task == TASK_MENU_TASK_4_AB_HOLD)
        {
            TaskMenuUi_SetStatusText("B TIME LOCKED");
        }
        else if ((s_selected_task == TASK_MENU_TASK_5_LAP_HOLD) ||
                 (s_selected_task == TASK_MENU_TASK_6_LAP_TARGET))
        {
            TaskMenuUi_SetStatusText("A TIME LOCKED");
        }
    }
}
...
{
    TaskMenuUi_SetElapsedMs(elapsed_ms);
    TaskMenuUi_SetStatusText(passed ? "PASS" : "FAIL");
    TaskMenuUi_SetState(TASK_MENU_STATE_FINISHED);
}
