 ...
 * 1700 = stable/level.
 */
#define SERVO_CENTER_US         1400U
#define SERVO_PUSH_MIN_US       1600U
 ...
 */
#define BALL_PUSH_KP            2.0f

/*
 * Braking: start braking when distance < |speed| * BRAKE_GAIN.
 * Higher = brake earlier.
 */
#define BALL_BRAKE_GAIN         8.0f

/*
 * Brake force = BASE + |speed| * SPEED_FACTOR.
 * Low base for gentle slow-speed braking; high factor for strong fast braking.
 */
#define BALL_BRAKE_BASE_US      60U
#define BALL_BRAKE_SPEED_FACTOR 3.0f

/*
 * Maximum brake pulse offset (us).
 */
#define BALL_BRAKE_MAX_US       300U
    ...
    if (s_vision_ok && BSP_VisionUart_HasNewDetection())
    {
        ...
        if (det.has_target)
        {
            ...
            else
            {
                ...
                if (moving_to_target)
                {
                    /* Always brake when moving toward target */
                    ...
                    /* Scale brake: light far away, heavy when close */
                    if ((int32_t)abs(s_last_error) > brake_dist)
                    {
                        /* Far away: use half brake as damping */
                        brake = (brake + 1) / 2;
                        s_braking = false;
                    }
                    else
                    {
                        /* Close: full brake to stop at target */
                        s_braking = true;
                    }
                ...
                /* Use wide limits when braking or stuck */
                clamped_pulse  = Servo_ClampPulse(raw_pulse,
                    s_braking || (s_stuck_boost > 0U));
