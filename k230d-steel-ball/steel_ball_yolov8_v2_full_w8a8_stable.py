"""CanMV K230D full-frame W8A8 entry with tracking and lead compensation.

Copy ``steel_ball_temporal_tracker.py`` to the SD-card root together with this
script.  A predicted frame keeps the existing SB protocol and uses
``score_milli=0`` so it remains distinguishable from a real observation.
"""
...
KMODEL_PATH = "/sdcard/steel_ball_yolov8n_320_v2_full_nncase211_w8a8.kmodel"
EXPECTED_KMODEL_SIZE = 3365584
...
def verify_kmodel_file():
    ...
    actual_size = stat[6]
    if actual_size != EXPECTED_KMODEL_SIZE:
        raise ValueError(
            "unexpected KModel size: expected=%d actual=%d"
            % (EXPECTED_KMODEL_SIZE, actual_size)
        )
    print(
        "[STEEL-V2-W8A8-STABLE] model_ok path=%s size=%d candidate_conf=%.2f"
        % (KMODEL_PATH, actual_size, CONFIDENCE_THRESHOLD)
    )
...
class SteelBallYoloV8(AIBase):
    ...
    def candidate_detections(self, result):
        ...
        for index in range(len(result[0])):
            ...
            detections.append(
                [x1, y1, x2, y2, float(result[2][index])]
            )
        return detections
    ...
    def draw_result(self, pipeline, detection):
        ...
        label = "steel_ball predicted" if score == 0.0 else "steel_ball %.2f" % score
...
def elapsed_ms(start, end):
    return time.ticks_diff(end, start) / 1000.0
...
def main():
    ...
    try:
        ...
        while True:
            ...
            capture_ready = time.ticks_us()
            result = detector.run(frame)
            inference_ready = time.ticks_us()
            candidates = detector.candidate_detections(result)
            detection = tracker.update_candidates(candidates)
            serial_detection = tracker.compensate(
                detection,
                SERIAL_COMPENSATION_MS,
                NOMINAL_FRAME_INTERVAL_MS,
            )
            display_detection = tracker.compensate(
                detection,
                DISPLAY_COMPENSATION_MS,
                NOMINAL_FRAME_INTERVAL_MS,
            )
            ...
            if frame_count % GC_INTERVAL_FRAMES == 0:
                ...
                gc.collect()
                gc_ms = elapsed_ms(gc_start, time.ticks_us())
            if frame_count % PERF_LOG_INTERVAL_FRAMES == 0:
                print(
                    "[STEEL-PERF] frame=%d source=%s capture_ms=%.1f "
                    "infer_ms=%.1f track_ms=%.1f display_ms=%.1f gc_ms=%.1f total_ms=%.1f mem_free=%d"
                    % (
                        frame_count,
                        tracker.last_source,
                        elapsed_ms(loop_start, capture_ready),
                        elapsed_ms(capture_ready, inference_ready),
                        elapsed_ms(inference_ready, tracking_ready),
                        elapsed_ms(tracking_ready, display_ready),
                        gc_ms,
                        elapsed_ms(loop_start, time.ticks_us()),
                        gc.mem_free(),
                    )
                )
